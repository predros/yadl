...
/**
 * @brief Controller class for the SourcePort model.
 */
class SourcePortController : public QAbstractTableModel {
    Q_OBJECT
public:
    /**
     * @brief Creates an SourcePortController instance.
    ...
    */
    ...
    /**
     * @brief Populates the controller with a JSON containing serialized
     * SourcePort instances. If any item is invalid, it is skipped.
     * @param array The JSON array containing the serialized instances.
     */
    void populate(const QJsonArray& array);

    /**
     * @brief Adds a new sourceport instance to the controller.
     * @param name The new sourceport's name.
     * @param path The new sourceport's file path.
     * @param params The new sourceport's command-line parameters.
     */
    void add(const QString& name, const QString& path, const QString& params);

    /**
     * @brief Edits an existing sourceport instance. Does nothing if the index
     * is out of bounds.
     * @param index The sourceport's index.
     * @param name The sourceport's new name.
     * @param path The sourceport's new file path.
     * @param params The sourceport's new command-line parameters.
     */
    void edit(int index, const QString& name, const QString& path, const QString& params);

    /**
     * @brief Removes an existing sourceport instance. If the index is
     * out-of-bounds, nothing is done.
     * @param index The sourceport's index.
     */
    void remove(int index);

    /**
     * @brief Moves a sourceport up the list. Does nothing if the index
     * is out of bounds or if the sourceport is already at the top.
     * @param index The sourceport's index.
     */
    ...
    /**
     * @brief Moves a sourceport up the list. Does nothing if the index
     * is out of bounds or if the sourceport is already at the bottom.
     * @param index The sourceport's index.
     */
